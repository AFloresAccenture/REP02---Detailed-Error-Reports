-- SELECTION QUERY FOR IN_TRANSIT
SELECT PLAN_ID                           
|| ',' ||TRANSACTION_ID                    
|| ',' ||INVENTORY_ITEM_ID                 
|| ',' ||ORGANIZATION_ID                   
...
|| ',' ||COLL_ORDER_TYPE                   
|| ',' ||PRODUCT_CLASSIFICATION  IN_TRANSIT_TABLE       
from msc_st_supplies where process_flag = 3 AND order_type in (11,12);

DAPAT where_clause     := process_flag = 3 AND order_type in (11,12) AND TRUNC(creation_date) = TRUNC(SYSDATE) AND abs(request_id) >= '||v_main_request_id;