-- SELECTION QUERY FOR ON_HAND_DATA
SELECT PLAN_ID                           
|| ',' ||TRANSACTION_ID                    
|| ',' ||INVENTORY_ITEM_ID                 
|| ',' ||ORGANIZATION_ID                   
...
from msc_st_supplies where process_flag = 3 AND order_type = 18;

DAPAT where_clause     := 'process_flag = 3 AND order_type = 18 AND TRUNC(creation_date) = TRUNC(SYSDATE) AND abs(request_id) >= '||v_main_request_id;