-- SELECTION QUERY FOR WORK_ORDERS
SELECT PLAN_ID                           
|| ',' ||TRANSACTION_ID                    
|| ',' ||INVENTORY_ITEM_ID                 
|| ',' ||ORGANIZATION_ID                   
...
|| ',' ||COLL_ORDER_TYPE                   
|| ',' ||PRODUCT_CLASSIFICATION  WORK_ORDERS_TABLE       
from msc_st_supplies where process_flag = 3 AND order_type = 3;

DAPAT where_clause     := process_flag = 3 AND order_type = 3 AND TRUNC(creation_date) = TRUNC(SYSDATE) AND abs(request_id) >= ||v_main_request_id;